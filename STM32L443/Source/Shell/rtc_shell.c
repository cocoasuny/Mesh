...
const char RTC_HelpMsg[] =
    ...
    " rtc wr alarm a <Hour>:<Minute>:<Second>                         - set alarm A\r\n"
    " rtc wr alarm b <Hour>:<Minute>:<Second>                         - set alarm B\r\n"
	"\r\n";
...
static void AlarmA_CB(void)
{
    printf("Test:Alarm A\r\n");
}

static void AlarmB_CB(void)
{
    printf("Test:Alarm B\r\n");
}
...
void Shell_RTC_Service(void)
{
    ...
    else if(StrComp(ptRxd,"wr alarm a "))
    {
        retval = sscanf((void*)shell_rx_buff,"%*s%*s%*s%*s %d:%d:%d",&Hour,&Min,&Sec); 
        if(retval != 3)
        {
            return;
        }
        ...
        {
            printf("Set Alarm A:%d:%d:%d\r\n",Hour,Min,Sec);
            ret = rtc_set_alarm_A(Hour,Min,Sec,AlarmA_CB);
            if(ret != HAL_OK)
            {
                printf("Alarm A Set Err:%d\r\n",ret);
            }
        }
    }
    else if(StrComp(ptRxd,"wr alarm b "))
    {
        retval = sscanf((void*)shell_rx_buff,"%*s%*s%*s%*s %d:%d:%d",&Hour,&Min,&Sec); 
        if(retval != 3)
        {
        ...
        {
            printf("Set Alarm B:%d:%d:%d\r\n",Hour,Min,Sec);
            ret = rtc_set_alarm_B(Hour,Min,Sec,AlarmB_CB);
            if(ret != HAL_OK)
            {
                printf("Alarm B Set Err:%d\r\n",ret);
            }
        }
    }    
